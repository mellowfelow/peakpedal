...
// force-invert dark emails and there's no reliable HTML opt-out. Table-based
// inline CSS for Outlook/Gmail/Zoho. Accent colours come from SITE.colors —
...
// opts: { title, preheader?, refBadge?, intro?, rows?, afterRows?, cta?, secondaryCta?, footer?, primaryColor? }
export function buildEmailHtml(opts) {
  const accent = opts.primaryColor || SITE.colors.primary;
  const rowsHtml = (opts.rows || []).map((r, i) => row(r, accent, i === 0)).join('');
...
<tr><td style="background:${SITE.colors.dark};padding:26px 32px;border-bottom:3px solid ${accent}">
<div style="font:800 20px/1.2 ${SANS};color:#fff">${escapeHtml(SITE.name)}</div>
</td></tr>
<tr><td style="padding:32px 32px 8px">
<h1 style="margin:0 0 12px;font:700 20px/1.3 ${SANS};color:${TEXT}">${escapeHtml(opts.title)}</h1>
${refPill}
${opts.intro ? `<p style="margin:0 0 8px;font:400 14px/1.6 ${SANS};color:${TEXT}">${opts.intro}</p>` : ''}
</td></tr>
...
${opts.footer ? escapeHtml(opts.footer) + '<br>' : ''}${escapeHtml(SITE.name)} · ${escapeHtml(SITE.domain)}
</td></tr>
